...
from netdiff.audit import RULES, audit, summarise
...
def test_every_rule_id_used_by_a_rule_exists_in_the_teaching_table():
    device = dev(ip="192.168.1.23", ports=[21, 23, 22, 1883, 8080])
    banners = {
    ...
    assert {f.rule for f in fired} == set(RULES)
...
def test_no_finding_leaves_an_unsubstituted_placeholder_anywhere():
    device = dev(ip="192.168.1.23", ports=[21, 22, 23, 1883, 8080])
    banners = {
        ("192.168.1.23", 22): "SSH-1.5-x",
        ("192.168.1.23", 8080): "HTTP/1.1 401\r\nWWW-Authenticate: Basic\r\n",
    }
    fired = audit(
        ...
        gw(
            [
                mapping(client="192.168.1.23", internal=8080),
                mapping(client="192.168.1.30", internal=9999),
                mapping(client="10.0.0.9"),
            ]
        ),
        banners,
    )
